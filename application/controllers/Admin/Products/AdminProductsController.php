...
//				}
				...
				$this->session->set_flashdata('success', 'Produit Ajouter avec succès');
	...
	public function delete($id)
	{
		if ($this->input->server('REQUEST_METHOD') == 'POST') {
			...
			$this->session->set_flashdata('info', 'Produit supprimé avec succès.');
			redirect(base_url('index.php/admin/products'));
			exit;
		}
	...
	public function validateAndUploadFiles()
	{
		...
		for ($i = 0; $i < $files_count; $i++) {
			...
			if ($_FILES['images']['size'] > $upload_config['max_size']) {
				...
				$this->form_validation->set_message('validateAndUploadFiles', 'la taille de l\'image n\'est pas supportée ' . $upload_config['max_size'] . ' Bytes');
				return false;
			}

			$current_mime_type = get_mime_by_extension($_FILES['images']['name']);
			if (!in_array($current_mime_type, $allowed_mime_types)) {
				$this->form_validation->set_message('validateAndUploadFiles', 'Le type du fichier ne pas être enregistrer');
				return false;
			}
